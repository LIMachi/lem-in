...
/*   Updated: 2018/01/30 06:17:38 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
inline static void	help(char *name)
{
	...
	"-path] [-h --hide-comments] [-l --ignore-double-link] [-d --debug]\n"
	...
	"or\n"
	"-d --debug:              print internal data after each passes of the bfs "
	"algorithm\n", name);
	exit(0);
}
...
inline static void	read_opts(int argc, char **argv, t_env_lem_in *e)
{
	...
	const t_getopt_opt	longopts[] = {{'h', "help", 0, 0}, {'i', "input", 1, 0},
		{'v', "verbose", 0, 0}, {'p', "path", 0, 0}, {'d', "debug", 0, 0},
		{'u', "hide-comments", 0, 0}, {'l', "ignore-double-link", 0, 0}};

	ge = ft_getopt_env("hvpuldi:", longopts);
...
int					main(int argc, char **argv)
{
	...
	.clean_output = 0, .ignore_double_link = 0, .debug = 0};
